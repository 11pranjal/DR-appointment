    ...
    <div className="page">
      <div className="card" style={{ marginBottom: '2rem' }}>
        <div style={{ display: 'flex', gap: '2rem', alignItems: 'flex-start' }}>
          <div className="doctor-avatar" style={{ fontSize: '4rem', width: '120px', height: '120px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {doctor.firstName[0]}
          </div>
          <div style={{ flex: 1 }}>
            <h1 style={{ marginTop: 0 }}>Dr. {doctor.firstName} {doctor.lastName}</h1>
            <p className="tag">{doctor.doctorProfile?.specialization}</p>
            <p><strong>Clinic:</strong> {doctor.doctorProfile?.clinicName || 'N/A'}</p>
            <p><strong>City:</strong> {doctor.doctorProfile?.city || 'N/A'}</p>
            <p><strong>Experience:</strong> {doctor.doctorProfile?.experience || 'N/A'}</p>
            <p><strong>Consultation Fee:</strong> Rs. {doctor.doctorProfile?.consultationFee || 'N/A'}</p>
            {doctor.doctorProfile?.bio && <p><strong>Bio:</strong> {doctor.doctorProfile.bio}</p>}
          ...
          {posts.map((post) => {
            ...
            return (
              <div className="doctor-card card" key={post._id}>
                ...
                <h3>{post.title}</h3>
                <p className="muted">Posted {new Date(post.createdAt).toLocaleDateString()}</p>
                <p>
                  {displayContent}
